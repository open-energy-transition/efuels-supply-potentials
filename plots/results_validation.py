...
def get_installed_capacity_pypsa(network):
    ...
    capacity_pypsa = (
        pd.concat([gen_capacities, storage_capacities], axis=0) / 1e3
    ).round(2)
    ...
    capacity_pypsa.rename(
        index={"nuclear": "Nuclear", "solar": "Solar", "biomass": "Biomass"},
        inplace=True,
    )
    ...
    # Filter and reformat
    capacity_pypsa = capacity_pypsa.loc[
        ["Nuclear", "Fossil fuels", "Hydro", "PHS", "Solar", "Wind", "Biomass"]
    ]
...
def get_generation_capacity_pypsa(network):
    ...
    """
    gen_capacities = (
        network.generators_t.p.multiply(network.snapshot_weightings.objective, axis=0)
    ...
    storage_capacities = (
        network.storage_units_t.p.multiply(
    ...
    generation_pypsa = (
        (pd.concat([gen_capacities, storage_capacities], axis=0)) / 1e6
    ).round(2)
    ...
    # Filter and reformat
    generation_pypsa = generation_pypsa.loc[
        [
            "Nuclear",
            "Fossil fuels",
            "PHS",
...
def plot_demand_validation(
    demand_ember, pypsa_demand, EIA_demand, horizon, country_code
):
    plt.figure(figsize=(8, 6))  # Set figure size
    bars = plt.bar(
        ["PyPSA", "Ember", "EIA"],
        [pypsa_demand, demand_ember, EIA_demand],
...
def plot_detailed_generation_validation(generation_df_, horizon):
    ax = generation_df_.plot(
        kind="bar", figsize=(12, 7), width=0.8, color=["#1f77b4", "#ff7f0e", "#2ca02c"]
    )
    plt.title(
...
def plot_capacity_validation(installed_capacity_df, horizon):
    ax = installed_capacity_df.plot(
        kind="bar", figsize=(12, 7), width=0.8, color=["#1f77b4", "#ff7f0e", "#2ca02c"]
    )
    plt.title(
        f"Comparison of Installed Capacity: PyPSA vs EMBER vs EIA, {horizon}",
        fontsize=16,
    )
...
def run(country_code, horizon):
    ...
    """
    # Get EIA data
    EIA_demand_path = os.path.join(DATA_DIR, "validation", "EIA_demands.csv")
    ...
    demand_df = pd.DataFrame(
        {
            "PyPSA data": [pypsa_demand],
            "Ember data": [demand_ember],
            "EIA data": [EIA_demand],
        }
    )
    ...
    installed_capacity_df = pd.concat(
        [pypsa_capacity, installed_capacity_ember, EIA_inst_capacities], axis=1
    ).fillna(0)

    # Save the output as a CSV file
    save_csv_output(
        installed_capacity_df,
        snakemake.output.capacity_csv,
        index=True,
        index_name="Installed capacities [GW]",
    )
    ...
    generation_df = pd.concat(
        [pypsa_generation, generation_data_ember, EIA_generation], axis=1
    ).fillna(0)
    generation_df
    ...
    save_csv_output(
        generation_df,
        snakemake.output.generation_csv,
        index=True,
        index_name="Generation [TWh]",
    )
    ...
    generation_df_ = pd.concat(
        [pypsa_generation_, generation_data_ember_, EIA_generation_], axis=1
    ).fillna(0)

    # Save the output as a CSV file
    save_csv_output(
        generation_df_,
        snakemake.output.generation_detailed_csv,
        index=True,
        index_name="Generation [TWh]",
    )

    # Plots
    plot_demand_validation(
        demand_ember, pypsa_demand, EIA_demand, horizon, country_code
    )
    plot_detailed_generation_validation(generation_df_, horizon)
    plot_capacity_validation(installed_capacity_df, horizon)
