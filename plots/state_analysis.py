...
def preprocess_eia_capacity(path, year):
    ...
    eia_cap_year = eia_cap_year.loc[
        eia_cap_year["Producer Type"] == "Total Electric Power Industry"
    ]
    ...
    eia_cap_year_grouped = eia_cap_year_grouped.drop(["all sources"], axis=1) / 1e3
...
def preprocess_pypsa_cap(network, state_mapping):
    ...
    series_gen_to_use = network.generators.groupby(["carrier", "bus"]).p_nom.sum()
    series_sto_to_use = network.storage_units.groupby(["carrier", "bus"]).p_nom.sum()
...
def preprocess_pypsa_demand(network, state_mapping):
    ...
    df = (
        network.loads_t.p_set.multiply(
...
def plot_installed_capacities(df, name, color):
    ...
    if name == "PyPSA":
        ax1.set_title("PyPSA")
        fig1.savefig(
            snakemake.output.statewise_installed_capacity_pypsa,
            dpi=plot_scale * 100,
            bbox_inches="tight",
        )
    elif name == "EIA":
        ax1.set_title("EIA")
        fig1.savefig(
            snakemake.output.statewise_installed_capacity_eia,
            dpi=plot_scale * 100,
            bbox_inches="tight",
        )
...
def plot_demand(df):
    ...
    # Save the figure
    fig1.savefig(
        snakemake.output.demand_statewise_comparison,
        dpi=plot_scale * 100,
        bbox_inches="tight",
    )
...
if __name__ == "__main__":
    ...
    # load solved network
    network = load_pypsa_network(
        snakemake.params.alternative_clustering, snakemake.input.solved_network
    )
    ...
    pypsa_installed_capacity_by_state.to_csv(
        snakemake.output.table_statewise_installed_capacity_pypsa
    )
    eia_installed_capacity_by_state.index.name = "capacities [GW]"
    eia_installed_capacity_by_state.to_csv(
        snakemake.output.table_statewise_installed_capacity_eia
    )
