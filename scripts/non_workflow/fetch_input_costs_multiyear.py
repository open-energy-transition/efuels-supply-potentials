...
def apply_hardcoded_model_assumptions(df):
    ...
    def replace_all_existing(tech, parameter, value, unit=None, note=None):
        ...
        if note:
            df.loc[mask, "source"] = (
                df.loc[mask, "source"].astype(str) + " | Model assumption: " + note
            )
...
def filter_to_long_format(
    df, used_technologies, scenario="Moderate", financial_case="Market"
):
    """
    ...
    if "scenario" in df.columns and "financial_case" in df.columns:
        mask = ((df["scenario"] == scenario) | df["scenario"].isna()) & (
            (df["financial_case"] == financial_case) | df["financial_case"].isna()
        )
        df_filtered = df[mask].copy()

    # Filter to only used technologies
    df_filtered = df_filtered[df_filtered["technology"].isin(used_technologies)].copy()
...
def categorize_technologies_long(df_long):
    ...
    h2_keywords = ["h2", "hydrogen", "electrolyzer", "electrolysis", "fuel cell"]
    ...
    remaining = df_long[
        ~df_long["technology"].isin(electricity_techs)
        & ~df_long["technology"].isin(hydrogen_ft_techs)
    ].copy()
    ...
    industrial_mask = (
        ...
        .str.lower()
        .apply(
            lambda x: any(kw in x for kw in industrial_keywords) or "cc" in x.lower()
        )
    )
    ...
    h2_mask = (
        remaining["technology"]
        .str.lower()
        .apply(lambda x: any(kw in x for kw in h2_keywords))
    )
    df_h2 = pd.concat([df_h2, remaining[h2_mask]], ignore_index=True)

    # Everything else goes to industrial
    df_industrial = pd.concat([df_industrial, remaining[~h2_mask]], ignore_index=True)
...
def get_technology_subcategories(df_long):
    ...
    for tech in df_long["technology"].unique():
        ...
        # Fossil fuels
            ...
            x in tech_lower for x in ["ccgt", "ocgt", "coal", "gas", "oil", "lignite"]
        ):
        ...
        # Industrial
            ...
            if "ethanol" in tech_lower:
                ...
                "steel" in tech_lower
                or "dri" in tech_lower
                or "bf-bof" in tech_lower
                or "eaf" in tech_lower
            ):
...
def write_excel_file_single(df_elec, df_h2, df_industrial, filename):
    ...
    sheet_names = [
        "1_Electricity_Generation",
        "2_Hydrogen_FischerTropsch",
        "3_Industrial_DAC",
    ]
...
def main():
    ...
    for scenario in scenarios:
        ...
        output_file = write_excel_file_single(df_elec, df_h2, df_industrial, filename)

        output_files.append((scenario, output_file, df_elec, df_h2, df_industrial))
